  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Eliminar','mmb').'" title="'.__('Eliminar','mmb').'" id="remove-'.$field['id'].'"></span>
    ...
    echo '</table><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Eliminar','mmb').'" title="'.__('Eliminar','mmb').'" id="remove-'.$field['id'].'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
      echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='Eliminar' type='button'/>";
    else
      echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='Subir imagen' type='button'/>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function addWysiwyg($id,$args,$repeater=false){
    $new_field = array('type' => 'wysiwyg','id'=> $id,'std' => '','desc' => '','style' =>'width: 300px; height: 300px','name' => 'WYSIWYG Editor Field', 'media_buttons' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
